             ...
             for aid in tqdm.tqdm(range(len(self.data['data']))):
                 ...
                 for pid in range(len(paragraphs)):
                     ...
                     for qid in range(len(qas)):
                         ...
                         question_file.write(' '.join(token.string for token in question_tokens)+'\n')
                         ...
                         stats_file.write( str(len(context_tokens)) + ',' + str(len(question_tokens))+'\n')
                         if(countExamples):
                             self.numTrainExamples += 1

    def visualizeData(self, filename="data/train.stats"):
        self.countContextLengths(filename)
        plt.figure()
        plt.hist(self.contextLengths,50)
        plt.show()
